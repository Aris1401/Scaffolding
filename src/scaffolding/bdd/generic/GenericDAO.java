package scaffolding.bdd.generic;
...
import scaffolding.bdd.annotations.ExcludeFromInsertion;
import scaffolding.bdd.annotations.ExcludeFromSelection;
import scaffolding.bdd.annotations.MakeQuery;
import scaffolding.bdd.annotations.PrimaryKey;
import scaffolding.bdd.annotations.Table;
import scaffolding.bdd.annotations.TransactionnalQuery;

public class GenericDAO<T> implements Serializable {
	private final String MAKE_QUERY_PARAMENTER_STARTER = "?";
	...
	public GenericDAO addToSetUpdate(String name, java.util.Date value) throws Exception {
		return addToSetUpdate(name, String.valueOf(value));
	}
	// ----------------------------------------------
	...
	public ArrayList<T> getFromDatabase(Connection c) throws SQLException, NoSuchMethodException, SecurityException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		...
		return executeQuery(c, selectionString);
	}
...
//						requete += "\'" + getterMethod.invoke(this) + "\'";
		...
		return executeUpdateQuery(c, requete.toString());
	}
	
	private int executeUpdateQuery(Connection c, String query) {
		int generatedKey = -1;
		
		try {
			
				PreparedStatement stat = c.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
	...
	private ArrayList<T> executeQuery(Connection c, String query) {
	...
	// Executing query annotation
	protected Object executeMakeQuery(Connection c, Object... parameters) {
		// Getting the calling method
		StackTraceElement[] stackTrace = Thread.currentThread().getStackTrace();
		String callingMethodName = stackTrace[2].getMethodName();
		
		// Getting the method by name
		Method callingMethod = null;
		Method[] objectMethods = currentClass.getDeclaredMethods();
		for (Method objectMethod : objectMethods) {
			if (objectMethod.getName().equals(callingMethodName)) {
				callingMethod = objectMethod;
				break;
			}
		}
		
		try {
			if (callingMethod == null) throw new Exception("Cas impossible.");			
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
		
		// Getting the query
		if (!callingMethod.isAnnotationPresent(MakeQuery.class)) throw new RuntimeException("Annotation imtrouvable.");
		MakeQuery queryAnnotation = callingMethod.getAnnotation(MakeQuery.class);
		if (queryAnnotation.query() == "") throw new RuntimeException("Aucune requete present.");
		
		// Checking if transactionnal
		boolean isTransactionnal = false;
		if (callingMethod.isAnnotationPresent(TransactionnalQuery.class)) isTransactionnal = true;
		
		// Get the function parameters and formatting query
		String query = queryAnnotation.query();
		for (int i = 0; i < parameters.length; i++) {
			String placeholder = MAKE_QUERY_PARAMENTER_STARTER + (i + 1);
			query = query.replace(placeholder, String.valueOf(parameters[i]));
		}
		...
		if (isTransactionnal) {
			queryResult = executeUpdateQuery(c, query);
		} else {
			ArrayList<T> databaseResults = executeQuery(c, query);
			if (!databaseResults.isEmpty()) {
				if (databaseResults.size() == 1) {
					queryResult = databaseResults.get(0);
				} else {
					queryResult = databaseResults;
				}
			}
		}
